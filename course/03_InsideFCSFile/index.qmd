...
You will notice at this point, the keyword names including a "$" symbol have stopped, so tick marks are no longer required. The only $ remaining is being used as a selector for a particular item in the list.
...
And finally, some of the values associated with the individual lasers particular values as far as delays and area scaling factors for a particular day (also useful when plotted). 
...
Depending on the arguments selected during `read.FCS()`, we might also encounter additional keywords that are added in by flowCore. For example
...
For some flow cytometry R packages, you will encounter these keywords integrated into the exported downstream .fcs file when opening it again in commercial software. It is likely the package authors forgot to set the transformation argument to FALSE, which is why these keywords persist. 
...
Next week, we will dive into the world of tidyverse, working specifically with these intermediate object types matrices and data.frames. If everything can in these objects can be thought of as rows and columns, can we select and filter for these more efficiently to get the data we need, and leave behind anything we don't? We will find out next week. 
